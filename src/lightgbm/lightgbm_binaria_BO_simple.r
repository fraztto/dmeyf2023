...
PARAM$experimento <- "BO-SIMPLE-13m-04u-3lag-v4-t5-hp2"

PARAM$input$dataset <- "./datasets/competencia_02_3lags.csv.gz"

 # los meses en los que vamos a entrenar
PARAM$input$training <- c(201905, 201906, 201907, 201908, 201909, 201910, 201911, 201912, 202011, 202012, 202101, 202102, 202103)
PARAM$input$validate <- c(202104)
PARAM$input$test <- c(202105)
...
PARAM$hyperparametertuning$semilla_azar <- 300581
#------------------------------------------------------------------------------
...
hs <- makeParamSet(
  ...
  makeIntegerParam("num_leaves", lower = 512L, upper = 1520L),
  makeIntegerParam("envios", lower = 14000L, upper = 20000L)
)
...
fganancia_logistic <- function(probs, vpesos) {
  # vector de ganancias
...
fganancia_logistic_lightgbm <- function(probs, datos) {
  ...
  return(fganancia_logistic(probs, vpesos))
}
#------------------------------------------------------------------------------
...
EstimarGanancia_lightgbm <- function(x) {
  ...
  # obtengo la ganancia
  ganancia <- unlist(modelocv$record_evals$val$ganancia$eval)[modelocv$best_iter]

  predict_test <- predict(
    modelocv,
    dtest$data #data.matrix(dtest[, campos_buenos, with = FALSE])
  )
...
dataset[
  foto_mes %in% PARAM$input$training | foto_mes %in% PARAM$input$validate | foto_mes %in% PARAM$input$test,
...
dataset[, training := 0L] # Usamos 0 para descartar, 1 para entrenar, 2 para validar y 3 para testear
dataset[
...
dataset[
  foto_mes %in% PARAM$input$validate,
  training := 2L
]
dataset[
  foto_mes %in% PARAM$input$test,
  training := 3L
]
...
dvalid <- lgb.Dataset(
  data = data.matrix(dataset[training == 2L, campos_buenos, with = FALSE]),
  label = dataset[training == 2L, clase01],
  weight = dataset[training == 2L, ifelse(clase_ternaria == "BAJA+2", 1.0000002, ifelse(clase_ternaria == "BAJA+1", 1.0000001, 1.0))],
  free_raw_data = FALSE
  )

dtest <- list(
  data = data.matrix(dataset[training == 3L, campos_buenos, with = FALSE]),
  label = dataset[training == 3L, clase01],
  weight = dataset[training == 3L, ifelse(clase_ternaria == "BAJA+2", 1.0000002, ifelse(clase_ternaria == "BAJA+1", 1.0000001, 1.0))]
)
