...
semillero <- c(100003, 100019, 100043, 100049, 100057,
               100069, 100103, 100109, 100129, 100151,
               100153, 100169, 100183, 100189, 100193,
               100207, 100213, 100237, 100267, 100271)
#------------------------------------------------------------------------------
...
for (semilla in semillero) {
  PARAM$lgb_basicos$seed <- semilla
  ...
  modelos[[as.character(semilla)]] <- modelo_train
}

# Realizar predicciones con cada modelo y promediar
predicciones <- matrix(0, nrow = nrow(dataset_test), ncol = length(modelos))
...
predicciones_promedio <- rowMeans(predicciones)
  ...
  tbl[, prob := predicciones_promedio]
  setorder(tbl, -prob)
  tbl[, gan_acum := cumsum(gan)]
